...
def patch_torch_diff_for_openvino():
    ...
    try:
        ...
        print("[OK] Patched torch.diff in masking_utils.find_packed_sequence_indices for OpenVINO compatibility")
    except Exception as e:
        print(f"[WARNING] Could not patch masking_utils: {e}")


@contextmanager
def _scoped_qwen3_export_patches():
    """Apply tracing-related transformer patches only within export scope."""
    masking_utils = None
    original_find_packed = None
    original_sdpa_mask = None
    original_eager_mask = None
    original_dynamic_layer_update = None

    try:
        if TORCH_AVAILABLE and MASKING_AVAILABLE:
            import transformers.masking_utils as masking_utils

            if hasattr(masking_utils, "find_packed_sequence_indices"):
                original_find_packed = masking_utils.find_packed_sequence_indices
            ...
            original_eager_mask = getattr(masking_utils, "eager_mask", None)
        ...
        if TORCH_AVAILABLE and DYNAMIC_LAYER_AVAILABLE and DynamicLayer is not None:
            original_dynamic_layer_update = DynamicLayer.update
            DynamicLayer.update = patched_dynamic_layer_update

        yield
    finally:
        if TORCH_AVAILABLE and MASKING_AVAILABLE and masking_utils is not None:
            if original_find_packed is not None:
                masking_utils.find_packed_sequence_indices = original_find_packed
            if original_sdpa_mask is not None:
                ALL_MASK_ATTENTION_FUNCTIONS.register("sdpa", original_sdpa_mask)
            if original_eager_mask is not None:
                ALL_MASK_ATTENTION_FUNCTIONS.register("eager", original_eager_mask)

        if original_dynamic_layer_update is not None:
            DynamicLayer.update = original_dynamic_layer_update


# Keep patch helpers defined in this module, but do not apply them at import
# time. Export-time patching should be scoped to patcher contexts.
...
# Do not globally register mask functions at module import time.
# Scoped export patchers are responsible for temporary registration.
...
def patch_stateful(ov_model, dim):
    # Select KV-cache inputs/outputs by name rather than by position. Positional
    # slicing (inputs[2:-1] / outputs[dim:]) assumes the KV tensors are the only
    # ones between the leading attention_mask/position_ids and a single trailing
    # input. The code predictor adds an extra trailing `generation_steps` input,
    # which shifts that slice and wrongly pulls `inputs_embeds` into the KV set.
    # fuse_cache_reorder then wraps inputs_embeds in a beam_idx Gather that
    # make_stateful never converts to a ReadValue, producing a malformed stateful
    # graph that breaks NPUW's StatefulToStateless pass ("no ReadValue is found at
    # the first input of Gather by `beam_idx` parameter"). Name-based selection is
    # robust to any number/order of non-KV inputs and outputs.
    key_value_input_names = [
        key.get_any_name() for key in ov_model.inputs if "past_key_values" in key.get_any_name()
    ]
    key_value_output_names = [
        out.get_any_name() for out in ov_model.outputs if out.get_any_name().startswith("present")
    ]
...
def convert_qwen3_tts_model(
    ...
    """
    Convert Qwen3-TTS auxiliary assets to OpenVINO format.

    Args:
        model_id: HuggingFace model ID or local path
        output_dir: Directory to save converted auxiliary models
    ...
    # Primary components are exported through the standard OpenVINO submodel path.
    # This helper now only exports optional speech-tokenizer artifacts.
    ...
    if speech_tokenizer_dir.exists():
        ...
        encoder_path = Path(speech_tokenizer_ov_dir) / SPEECH_TOKENIZER_ENCODER_NAME
        decoder_path = Path(speech_tokenizer_ov_dir) / SPEECH_TOKENIZER_DECODER_NAME
        try:
            convert_speech_tokenizer(
                str(speech_tokenizer_dir),
                speech_tokenizer_ov_dir,
                quantization_config=quantization_config,
                use_local_dir=use_local_dir,
            )
        except Exception as exception:
            print(f"⚠️ Speech tokenizer conversion failed in-process. Details: {exception}")
            if encoder_path.exists() and decoder_path.exists():
                print("✅ Speech tokenizer artifacts were produced before the in-process failure")
            else:
                print("⚠️ Speech tokenizer conversion failed; falling back to PyTorch speech tokenizer during inference.")
    else:
        print(f"ℹ️ No speech tokenizer found in model. Using PyTorch version during inference.")


def convert_speech_tokenizer(model_id, output_dir, quantization_config=None, use_local_dir=False):
    """
    ...
    with _scoped_qwen3_export_patches():
        ...
        if not decoder_path.exists():
            ...
            # Patch masking helpers used during tracing/export to avoid the
            # transformers vmap path that is incompatible with torch.jit.trace.
            ...
            _orig_qwen_causal = _qwen_modeling.create_causal_mask
            _orig_qwen_sliding = getattr(_qwen_modeling, "create_sliding_window_causal_mask", None)
            ...
            _qwen_modeling.create_causal_mask = _simple_causal_mask
            if _orig_sliding:
                _masking_utils.create_sliding_window_causal_mask = _simple_sliding_window_causal_mask
            if _orig_qwen_sliding:
                _qwen_modeling.create_sliding_window_causal_mask = _simple_sliding_window_causal_mask

            try:

                class DecoderDirectBTQ(torch.nn.Module):
                ...
                decoder_wrapper = DecoderDirectBTQ(decoder)

                num_quantizers = tokenizer_model.config.decoder_config.num_quantizers
                # Trace at 256 tokens = chunk_size(231) + left_context(25), matching chunked_decode
                example_input = torch.randint(0, 2048, [1, 256, num_quantizers], dtype=torch.long)
                ...
                _qwen_modeling.create_causal_mask = _orig_qwen_causal
                if _orig_sliding:
                    _masking_utils.create_sliding_window_causal_mask = _orig_sliding
                if _orig_qwen_sliding:
                    _qwen_modeling.create_sliding_window_causal_mask = _orig_qwen_sliding
    ...
    def forward(
        ...
        **kwargs,
        ...
        # Generation stage
            ...
            _do_sample = subtalker_dosample if subtalker_dosample is not None else True
            predictor_result = self.code_predictor.generate(
                inputs_embeds=torch.cat((past_hidden, last_id_hidden), dim=1),
                max_new_tokens=self.config.num_code_groups - 1,
                do_sample=_do_sample,
                top_p=subtalker_top_p if _do_sample else None,
                top_k=subtalker_top_k if _do_sample else None,
                temperature=subtalker_temperature if _do_sample else 1.0,
...
class OVQwen3TTSModel:
    ...
    def _sample_next_token(
        ...
        print("_sample_next_token...", do_sample)
    ...
    def extract_voice_clone_embeddings(
        self,
        ref_audio,
        ref_text=None,
        x_vector_only_mode=False,
        output_speaker_embedding_file=None,
        output_ref_code_file=None,
    ):
        """
        Extract and optionally save speaker embedding and reference codes from reference audio/text.
        
        Args:
            ref_audio: Path to reference audio file.
            ref_text: Reference text for ICL mode (optional).
            x_vector_only_mode: If True, only extract speaker embedding (no ref codes).
            output_speaker_embedding_file: Path to save speaker embedding binary file.
            output_ref_code_file: Path to save reference codes numpy file.
        
        Returns:
            Dict with 'ref_spk_embedding' (numpy array) and optionally 'ref_code'
        """
        if self.tts_model_type != "base":
            raise ValueError(f"Model type {self.tts_model_type} does not support voice cloning.")
        ...
        # Extract speaker embedding
        if "ref_spk_embedding" in prompt_item:
            spk_embed = prompt_item["ref_spk_embedding"]
            if isinstance(spk_embed, torch.Tensor):
                spk_embed_np = spk_embed.cpu().numpy()
            else:
                spk_embed_np = spk_embed
            
            result["ref_spk_embedding"] = spk_embed_np
            
            # Save to file if requested
            if output_speaker_embedding_file:
                spk_embed_np.tofile(output_speaker_embedding_file)
                print(f"[Info] Speaker embedding saved to: {output_speaker_embedding_file}")
                print(f"[Info] Embedding shape: {spk_embed_np.shape}")
        
        # Extract ref codes
        if "ref_code" in prompt_item and prompt_item["ref_code"] is not None:
            ref_code = prompt_item["ref_code"]
            if isinstance(ref_code, torch.Tensor):
                ref_code_np = ref_code.cpu().numpy()
            else:
                ref_code_np = ref_code
            
            result["ref_code"] = ref_code_np
            
            # Save to file if requested
            if output_ref_code_file:
                import numpy as np
                np.save(output_ref_code_file, ref_code_np)
                print(f"[Info] Reference codes saved to: {output_ref_code_file}")
                print(f"[Info] Ref code shape: {ref_code_np.shape}")
        ...
        **kwargs,
        ...
        for idx in range(batch_size):
            ...
            if icl_mode and ref_id is not None:
                # Keep ICL packing consistent with the corrected C++ path.
                # Even when API flag uses streaming mode, voice-clone ICL should use
                # non-streaming-style text/code layout to avoid long/silent outputs.
                icl_non_streaming_mode = True
